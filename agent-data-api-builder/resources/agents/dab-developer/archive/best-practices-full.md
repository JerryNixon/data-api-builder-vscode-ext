...
**Always minimize costs, especially for development and testing environments.**

#### Use Lowest SKUs

| Resource | Development SKU | Production SKU | Monthly Cost (Dev) |
|----------|----------------|----------------|-------------------|
| Azure Container Registry | Basic | Standard | ~$5 |
| Azure SQL Database | Basic (5 DTU) | Standard S1+ | ~$5 |
| Azure Container Apps | Consumption | Consumption | Pay-per-use |
| Log Analytics | Free tier | Standard | $0 (limited) |

**Example: Low-cost deployment commands:**
```bash
# ACR - Basic tier
az acr create --name myacr --resource-group rg-dev --sku Basic

# Azure SQL - Basic tier (5 DTUs)
az sql db create --name mydb --server myserver --resource-group rg-dev \
  --edition Basic --capacity 5

# ACA uses consumption by default (scales to zero)
```

#### Mandatory Cleanup Reminder

**Always include this warning after any Azure deployment:**
...
1. **Delete entire resource group** - Fastest, removes everything
2. **Stop ACA apps** - `az containerapp update --name myapp --scale-rule-min 0`
3. **Pause Azure SQL** - Use serverless tier with auto-pause
4. **Set expiration tags** - Tag resources with `delete-by=2024-12-31`

#### Cost-Saving Tips

- **Use serverless Azure SQL** - Auto-pauses when idle
- **ACA scales to zero** - No cost when no requests
- **Share ACR across projects** - One Basic ACR for multiple dev projects
