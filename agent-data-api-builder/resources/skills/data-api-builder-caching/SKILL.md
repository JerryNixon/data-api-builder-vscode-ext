...
2. Opt each entity in via its `cache` block; pick `L1` (in-memory only) or `L1L2` (default, adds distributed).
3. For multi-instance deployments, add `runtime.cache.level-2` with `provider: redis`, `connection-string`, and optional `partition`.
4. Validate with REST calls; use `Cache-Control: no-cache` to force refresh, `no-store` to bypass write-back.
...
- Caching applies to REST only; GraphQL request-level cache directives are not honored.
